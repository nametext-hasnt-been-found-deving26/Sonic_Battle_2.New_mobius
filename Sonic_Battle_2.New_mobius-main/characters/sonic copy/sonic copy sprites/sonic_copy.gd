...
# Heavy Attack (Sonic Flare) System
...
# Sonic Flare Settings
const FLARE_DROP_SPEED = 20.0
const FLARE_HANDSTAND_DURATION = 0.15
const FLARE_SWEEP_DURATION = 0.4
const FLARE_LEG_LIFT_DURATION = 0.25
const FLARE_RECOVERY_DURATION = 0.3
const FLARE_RECOVERY_LIFT = 3.0
const FLARE_RECOIL_PUSHBACK = 2.0

# Pathfinding
const PATH_UPDATE_RATE = 0.2
const JUMP_CHECK_RATE = 0.5
...
func _physics_process(delta):
	...
	# Apply gravity (skip during grounded heavy attack phases)
	if current_state == State.HEAVY_ATTACK:
		if heavy_phase in [HeavyPhase.HANDSTAND, HeavyPhase.FLARE, HeavyPhase.LEG_LIFT]:
			pass  # No gravity during grounded phases
	...
	# Face player (not during heavy attack)
	if current_state != State.HEAVY_ATTACK:
...
func _handle_heavy_attack_state(delta: float, target: CharacterBody3D, direction_2d: Vector2):
	...
	match heavy_phase:
		HeavyPhase.DROP:
			# Phase 1: Initial Drop - fall straight down fast
			velocity.x = 0
			velocity.z = 0
			...
			if is_on_floor():
				velocity.y = 0
				heavy_phase = HeavyPhase.HANDSTAND
				heavy_phase_timer = FLARE_HANDSTAND_DURATION
		
		HeavyPhase.HANDSTAND:
			# Phase 2: Handstand - brief pause on one hand
			velocity = Vector3.ZERO
			heavy_phase_timer -= delta
			
			if heavy_phase_timer <= 0:
				heavy_phase = HeavyPhase.FLARE
				heavy_phase_timer = FLARE_SWEEP_DURATION
		
		HeavyPhase.FLARE:
			# Phase 3: Sweeping Kick - stay completely still
			velocity = Vector3.ZERO
			heavy_phase_timer -= delta
			
			if heavy_phase_timer <= 0:
				# Deal damage during the flare
				_perform_attack(target)
				heavy_phase = HeavyPhase.LEG_LIFT
				heavy_phase_timer = FLARE_LEG_LIFT_DURATION
		
		HeavyPhase.LEG_LIFT:
			# Phase 4: Leg Lift - shift weight, legs up high
			velocity = Vector3.ZERO
			heavy_phase_timer -= delta
			
			if heavy_phase_timer <= 0:
				heavy_phase = HeavyPhase.RECOVERY
				heavy_phase_timer = FLARE_RECOVERY_DURATION
				# Hop back into neutral stance - slight up and backward
				velocity.y = FLARE_RECOVERY_LIFT
				velocity.x = -current_facing * FLARE_RECOIL_PUSHBACK
		
		HeavyPhase.RECOVERY:
			# Phase 5: Recovery - airborne, returning to neutral
			heavy_phase_timer -= delta
...
func _change_state(new_state: State) -> void:
	...
	# Don't interrupt heavy attack until recovery phase
	if current_state == State.HEAVY_ATTACK:
		if heavy_phase != HeavyPhase.RECOVERY:
			return
	...
	match new_state:
		...
		State.HEAVY_ATTACK:
			heavy_phase = HeavyPhase.DROP
			heavy_phase_timer = 0.0
		State.RECOVERING:
			state_timer = 0.3
			attack_cooldown = 1.2
