...
@export var initial_dash_time: float
@export var dash_time_multiplier: float
var current_direction: String
var running_start: bool = false
var dash_timer: float
var apply_dash_friction: bool = false
var can_rengage_dash: bool = true
...
func handle_state(delta: float) -> void:
	...
	match current_state:
		...
		State.DASHING: _apply_dashing (delta, direction)
...
func handle_idle_state(delta: float, input_dir: Vector2, direction: Vector3) -> void:
	...
	if Input.is_action_just_pressed("dash_p1"):
		dash_timer = initial_dash_time
		running_start = true
	elif Input.is_action_pressed("dash_p1"):
		dash_timer += dash_time_multiplier
	if input_dir != Vector2.ZERO:
...
func handle_running_state(delta: float, input_dir: Vector2, direction: Vector3) -> void:
	if not Input.is_action_pressed("dash_p1"):
		can_rengage_dash = true
	if Input.is_action_just_pressed("dash_p1"):
		running_start = true
		dash_timer = initial_dash_time
	...
	elif Input.is_action_pressed("dash_p1") and dash_timer >= 0 and can_rengage_dash == true :
...
func _apply_dashing (delta: float ,direction: Vector3) -> void:
	dash_timer -= delta
	if direction and dash_timer >= 0:
		if running_start == true:
			anim_change = "Dashing toward opponent "
			velocity.x = direction.x * (max_speed + 1.75)
			velocity.z =  direction.z * (max_speed + 1.25)
			running_start = false
		else:
			if animated_sprite_3d.frame == 4 and anim_change == "Dashing toward opponent " :
				print("true")
				anim_change = "Dashing"
...
func get_animation_name(state: State) -> String:
	match state:
		...
		State.DASHING: return "Dashing toward opponent " if dash_timer <= initial_dash_time + 0.3 else "Dashing"
