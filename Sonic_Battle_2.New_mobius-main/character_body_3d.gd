...
enum State {
	...
	AIR_ACTION, AIR_ATTACK, PURSUIT_ATTACK, DASHING_TOWARD_OPPONENT,
...
@export var dashing_toward_opponent_duration: float = 0.5
@export var turning_duration: float = 0.15
@export var turning_while_running_duration: float = 0.2
...
var dashing_toward_opponent_timer: float = 0.0
...
func validate_animations() -> void:
	if animated_sprite_3d and animated_sprite_3d.sprite_frames:
		...
		for anim in animations:
			print(" - '", anim, "'")
		print("=============================")
		var critical_anims = ["1st attack", "2nd attack", "3rd attack", "Heavy attack", "Upper attack", "Air action", "Air attack "]
...
func handle_attack_input() -> void:
	if current_state == State.HEAVY_ATTACK or heavy_attack_timer > 0.0:
		return
...
func process_attack_input() -> void:
	# Check if we can air attack first, regardless of floor state
...
func handle_state(delta: float) -> void:
	...
	match current_state:
		...
		State.DASHING_TOWARD_OPPONENT:
			handle_dashing_toward_opponent_state(delta, direction)
		State.GUARD:
		...
		State.TURNING_AROUND_WHILE_RUNNING:
			handle_turning_around_while_running_state(delta, input_dir)
		State.STOPPING:
			handle_stopping_state(delta, input_dir)
		State.WALL_CLING:
			handle_wall_cling_state(delta, input_dir)
	if input_dir != Vector2.ZERO:
		last_input_direction = input_dir

func update_facing_direction(input_dir: Vector2) -> void:
	# Only update facing during states that allow direction change
	if current_state not in [State.TURNING_AROUND, State.TURNING_AROUND_WHILE_RUNNING, State.ATTACK_1, State.ATTACK_2, State.ATTACK_3, State.HEAVY_ATTACK]:
		if input_dir.x != 0:
...
func handle_idle_state(delta: float, input_dir: Vector2, direction: Vector3) -> void:
	...
	# Always check if we're not on floor and transition to FALLING
	if not is_on_floor():
...
func handle_falling_state(delta: float, direction: Vector3) -> void:
	apply_air_control(direction)
	# Make sure we never transition to IDLE while falling
	if is_on_floor():
...
func handle_air_attack_state(delta: float, direction: Vector3) -> void:
	...
	if air_attack_timer <= 0:
		...
		# Always go to falling when air attack ends, never idle
		change_state(State.FALLING)
...
func handle_dashing_toward_opponent_state(delta: float, direction: Vector3) -> void:
	# Dash forward at high speed to pursue
	var dash_speed = max_speed * 1.8
	velocity.x = current_facing_direction * dash_speed
	
	dashing_toward_opponent_timer -= delta
	
	# Can cancel into pursuit attack
	if Input.is_action_just_pressed("p"):
		change_state(State.PURSUIT_ATTACK)
		return
	
	if dashing_toward_opponent_timer <= 0:
		if not is_on_floor():
			change_state(State.FALLING)
...
func handle_guard_state(delta: float, input_dir: Vector2) -> void:
	...
	# Handle heavy attack from guard
	...
	if Input.is_action_pressed("shift"):
		...
		heal_loop_timer = 0.0
		# Only exit guard when shift is released
		if not is_on_floor():
			change_state(State.FALLING)
		elif input_dir != Vector2.ZERO:
			change_state(State.RUNNING)
...
func handle_turning_around_while_running_state(delta: float, input_dir: Vector2) -> void:
	# Keep some momentum during running turn
	velocity.x = move_toward(velocity.x, 0, friction * 1.5)
	velocity.z = move_toward(velocity.z, 0, friction * 1.5)
...
func handle_wall_cling_state(delta: float, input_dir: Vector2) -> void:
	# Stick to wall - zero out velocity
	...
	if Input.is_action_just_pressed("ui_accept"):
		...
		velocity.x = current_facing_direction * -max_speed  # Push away from wall
		change_state(State.JUMPING)
		return
	
	# Fall off wall after timer
...
func can_air_attack() -> bool:
	# Allow air attack in more states and fix the condition logic
	return (current_state in [State.JUMPING, State.FALLING, State.AIR_ACTION] and 
			attack_cooldown <= 0 and 
			current_state != State.HEAVY_ATTACK and 
			air_attack_cooldown <= 0.0 and 
...
func exit_current_state(new_state: State) -> void:
	match current_state:
		...
		State.DASHING_TOWARD_OPPONENT:
			dashing_toward_opponent_timer = 0.0
		State.TURNING_AROUND:
			turning_timer = 0.0
		State.TURNING_AROUND_WHILE_RUNNING:
			turning_while_running_timer = 0.0
		State.STOPPING:
			stopping_timer = 0.0
		State.WALL_CLING:
...
func get_animation_name(state: State) -> String:
	match state:
		...
		State.DASHING_TOWARD_OPPONENT: return "Dashing toward opponent"
...
func set_state_timer(state: State) -> void:
	match state:
		...
		State.DASHING_TOWARD_OPPONENT: dashing_toward_opponent_timer = dashing_toward_opponent_duration
	...
	if animated_sprite_3d and animated_sprite_3d.sprite_frames:
		if animated_sprite_3d.sprite_frames.has_animation(animation_name):
			...
			if animated_sprite_3d.sprite_frames.has_animation(fallback):
				...
				print("WARNING: Animation '", animation_name, "' not found and no fallback available!")
	else:
		print("ERROR: No SpriteFrames resource assigned to AnimatedSprite3D!")
